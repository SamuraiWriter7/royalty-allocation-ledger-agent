...
def semantic_errors(
    ...
    duplicates = sorted(
        {
            ...
            if beneficiary_ids.count(beneficiary_id) > 1
        }
    )
    ...
    gross_allocated = sum(
        float(item["gross_allocation_amount"])
        for item in beneficiaries
    )
    ...
    for index, item in enumerate(beneficiaries):
        ...
        if (
            ...
            errors.append(
                f"beneficiaries[{index}]: "
                "payable_after_approval cannot contain "
                "a held amount"
            )

    if not close_enough(
        weight_total,
        float(summary["declared_weight_total"]),
    ):
        errors.append(
            "integrity_summary.declared_weight_total "
            "does not match beneficiary weights"
        )

    if not close_enough(
        payable_total,
        float(summary["declared_payable_amount"]),
    ):
        errors.append(
            "integrity_summary.declared_payable_amount "
            "does not match beneficiary payable amounts"
        )

    if not close_enough(
        held_total,
        float(summary["declared_held_amount"]),
    ):
        errors.append(
            "integrity_summary.declared_held_amount "
            "does not match beneficiary held amounts"
        )

    if not close_enough(
        gross_allocated,
        float(summary["declared_allocated_amount"]),
    ):
        errors.append(
            "integrity_summary.declared_allocated_amount "
            "does not match beneficiary allocations"
        )

    reserve_total = (
        float(reserve["holdback_amount"])
        + float(reserve["rounding_remainder_amount"])
        + float(reserve["unallocated_amount"])
    )
    ...
    if not close_enough(
        ...
        errors.append(
            "beneficiary allocations plus all reserve "
            "amounts must equal "
    ...
    if (
        ...
        errors.append(
            "settlement cannot be approved_for_handoff "
            "before human review approval"
        )

    if settlement["execution_prohibited"] is not True:
        errors.append(
            "v0.1 requires "
            "settlement_state.execution_prohibited "
            "to remain true"
        )
...
def validate_expected_fail(
    ...
    print(
        f"[FAIL] invalid example was accepted: "
        f"{path.relative_to(ROOT)}"
    )
